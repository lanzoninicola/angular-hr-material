...
export class RequestToHireFormService {
  ...
  /**
   *
   * Getting the options for the select controls
   *
   */

  get statusSelectOptions(): Observable<SelectOptionConfig[]> {
    return this._getSelectOptionsFromPicklist('rth-working-status');
  }

  get businessUnitSelectOptions(): Observable<SelectOptionConfig[]> {
    return this._getSelectOptionsFromPicklist('business-unit');
  }

  get employmentStatusSelectOptions(): Observable<SelectOptionConfig[]> {
    return this._getSelectOptionsFromPicklist('employment-status');
  }

  get roleLevelSelectOptions(): Observable<SelectOptionConfig[]> {
    return this._getSelectOptionsFromPicklist('role-level');
  }

  get jobLocationTypeSelectOptions(): Observable<SelectOptionConfig[]> {
    return this._getSelectOptionsFromPicklist('job-location-type');
  }

  get jobLocationSelectOptions(): Observable<SelectOptionConfig[]> {
    return this._getSelectOptionsFromPicklist('job-location');
  }

  get yesNoSelectOptions(): Observable<SelectOptionConfig[]> {
    return this._getSelectOptionsFromPicklist('yesno');
  }

  get departmentsSelectOptions(): Observable<SelectOptionConfig[]> {
    return this._getSelectOptionsFromModel(this._departmentService);
  }

  get branchesSelectOptions(): Observable<SelectOptionConfig[]> {
    return this._getSelectOptionsFromModel(this._branchService);
  }

  get jobRolesSelectOptions(): Observable<SelectOptionConfig[]> {
    return this._getSelectOptionsFromModel(this._jobRoleService);
  }

  private _getSelectOptionsFromPicklist(
    type: string
  ): Observable<SelectOptionConfig[]> {
    return this._picklistService.findByType(type).pipe(
      map((picklist: PicklistModel) => {
        return picklist.items.map((item) => {
          return {
            value: item.getId(),
  ...
  private _getSelectOptionsFromModel(dataService: any) {
    return dataService.findAll().pipe(
      map((models: any[]) => {
